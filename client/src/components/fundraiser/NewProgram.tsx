...
const NewProgram = ({ openNewProgram, setOpenNewProgram }: IProps) => {
  ...
  return (
    <div className={`fixed top-0 left-0 bottom-0 right-0 z-50 bg-[rgba(0,0,0,.8)] transition ${openNewProgram ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
      <div ref={newProgramRef} className={`absolute top-0 right-0 sm:w-[400px] w-[300px] overflow-auto hide-scrollbar bg-white h-screen transition origin-right ${openNewProgram ? 'scale-x-100 pointer-events-auto opacity-100' : 'scale-x-0 pointer-events-none opacity-0'} p-5`}>
