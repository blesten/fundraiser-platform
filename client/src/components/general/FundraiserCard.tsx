...
import NewProgram from '../fundraiser/NewProgram'
import ActivityLog from '../fundraiser/ActivityLog'
import Delete from './Delete'
import WithdrawFund from '../fundraiser/WithdrawFund'
...
const FundraiserCard = ({ image, title, description, collectedFund, goals, donatorCount, isAdmin }: IProps) => {
  ...
  const [openWithdrawFund, setOpenWithdrawFund] = useState(false)
  ...
  return (
    <>
          ...
          <div className='mb-3 flex items-center justify-between'>
            ...
            <div ref={controlPanelRef} className='relative'>
              ...
              <div className={`absolute top-full right-0 mt-1 border border-gray-300 shadow-xl rounded-lg bg-white w-[170px] ${openControlPanel ? 'scale-y-100 pointer-events-auto' : 'scale-y-0 pointer-events-none'} transition origin-top`}>
                <div onClick={() => setOpenNewProgram(true)} className='flex items-center gap-3 p-3 cursor-pointer hover:bg-gray-100 rounded-t-lg'>
                  <MdModeEdit className='text-orange-600' />
                  <p className='text-gray-600 text-sm'>Edit</p>
                </div>
                <div onClick={() => setOpenWithdrawFund(true)} className='flex items-center gap-3 p-3 cursor-pointer hover:bg-gray-100 rounded-t-lg'>
                  <IoIosSend className='text-blue-600' />
                  <p className='text-gray-600 text-sm'>Withdraw Funds</p>
                </div>
                <div onClick={() => setOpenActivityLog(true)} className='flex items-center gap-3 p-3 cursor-pointer hover:bg-gray-100 rounded-t-lg'>
                ...
                <div onClick={() => setOpenDelete(true)} className='flex items-center gap-3 p-3 cursor-pointer hover:bg-gray-100 rounded-t-lg'>
      ...
      <WithdrawFund
        openWithdrawFund={openWithdrawFund}
        setOpenWithdrawFund={setOpenWithdrawFund}
      />
